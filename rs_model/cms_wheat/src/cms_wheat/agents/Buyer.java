...
//		slopeOfTheDemandFunction=(int)(1*averageConsumption/possiblePrices.get(possiblePrices.size()-1));
		slopeOfTheDemandFunction=(int)(Cms_builder.demandFunctionSlopeTuner*averageConsumption/Cms_builder.demandFunctionReferencePrice);
//		System.out.println("Created buyer:    "+name+", latitude: "+latitude+", longitude: "+longitude+" average consumption "+averageConsumption+" tuner "+Cms_builder.demandFunctionInterceptTuner+" initialInterceptOfTheDemandFunction "+initialInterceptOfTheDemandFunction);
		if(Cms_builder.verboseFlag){System.out.println("Created buyer:    "+name+", latitude: "+latitude+", longitude: "+longitude+" average consumption "+averageConsumption+" minimum consumption "+minimumConsumption+" maximum consumption "+maximumConsumption+" stock "+stock);}
...
//Timing guerra 
//inizio marzo
//con 24 mesi inizializzazione e dati dal 1993 al 2016 
//303 march 2016
			if(RepastEssentials.GetTickCount()>302 && RepastEssentials.GetTickCount()<421){
				importAllowed=false;
System.out.println(RepastEssentials.GetTickCount()+" time "+name+" importFlag: "+importAllowed);
			}
		...
		if(demandFunctionParametersList.size()>0){
			...
			if(latestContractsInPossibleMarketSessionsList.size()<1){  //buyers with no producer can find all the markets closed and had no contracts in the previous period
				if(startBuyingMarketSessionsList.size()<1){
					...
					if(Cms_builder.verboseFlag){System.out.println("          "+name+":gap to target "+gapToTarget);}
					for(MarketSession aMarketSession : startBuyingMarketSessionsList){
						for(DemandFunctionParameters aParametersHolder : demandFunctionParametersList){
							if(aMarketSession.getMarketName().equals(aParametersHolder.getMarketName()) && aMarketSession.getProducerName().equals(aParametersHolder.getProducerName())){
				...
				if(latestContractsInPossibleMarketSessionsList.size()>1){ 
								...
								for(int i=latestContractsInPossibleMarketSessionsList.size()-1;i>decreaseQuantityProducerPositionInPriceRanking-1;i--){
									...
									for(DemandFunctionParameters aParametersHolder : demandFunctionParametersList){
										if(tmpContract.getMarketName().equals(aParametersHolder.getMarketName()) && tmpContract.getProducerName().equals(aParametersHolder.getProducerName())){
											...
											//	aParametersHolder.setSlope((tmpIntValue*Cms_builder.demandFunctionSlopeTuner)/(Cms_builder.demandFunctionReferencePrice*(1+Cms_builder.demandFunctionSlopeTuner)));
											aParametersHolder.slopeNotChanged();
										}
					...
					if(Cms_builder.allowDemadsShiftAccordingToGapToTarget){ //start flag to shift demand in accordance to gapToTarget
						...
						for(MarketSession aMarketSession : possibleMarketSessionsList){
							for(DemandFunctionParameters aParametersHolder : demandFunctionParametersList){
								if(aMarketSession.getMarketName().equals(aParametersHolder.getMarketName()) && aMarketSession.getProducerName().equals(aParametersHolder.getProducerName())){
									...
									tmpIntValue=aParametersHolder.getIntercept();
									//slope is obtained by imposing the condition that at p=5 asked quantity is equal to intercept/(1+slopeTuner)
									//aParametersHolder.setSlope((tmpIntValue*Cms_builder.demandFunctionSlopeTuner)/(Cms_builder.demandFunctionReferencePrice*(1+Cms_builder.demandFunctionSlopeTuner)));
									aParametersHolder.slopeNotChanged();
								}
							}
					...
					} //end flag to shift demand in accordance to gapToTarget
					else{
						if(Cms_builder.verboseFlag){System.out.println("              Demands not shifted. set allowDemadsShiftAccordingToGapToTarget=true to anable shifting");}
					}
...
//	System.out.println("slope tuner "+Cms_builder.demandFunctionSlopeTuner);
//	System.out.println("intercept tuner "+Cms_builder.demandFunctionInterceptTuner);
//System.out.println("zone consumption "+myZoneInfoHolder.getConsumption());
//System.out.println("zone production "+myZoneInfoHolder.getProduction());
//System.out.println("                 "+myZoneInfoHolder.getName()+" setting up domestic demand curve ");
...
//						if(myZoneInfoHolder.getExcessSupply()>0){
//							initialInterceptOfTheDemandFunction=(int)(1.5*myZoneInfoHolder.getConsumption());
							aParametersHolder=new DemandFunctionParameters(initialInterceptOfTheDemandFunction,aMarketSession.getMarketName(),aMarketSession.getProducerName());
													aParametersHolder.setSlope(Cms_builder.demandFunctionSlopeTuner*Math.min(myZoneInfoHolder.getConsumption(),myZoneInfoHolder.getProduction())/Cms_builder.demandFunctionReferencePrice);
//							aParametersHolder.setSlope(0.5*myZoneInfoHolder.getConsumption()/5);
//						}
						/*
						else{//this is to decrease domestic demand by the imported quantity, otherwise domestic prices can be too high
							initialInterceptOfTheDemandFunction=(int)(1.5*(myZoneInfoHolder.getConsumption()-myZoneInfoHolder.getExcessDemand()*1.7));
							aParametersHolder=new DemandFunctionParameters(initialInterceptOfTheDemandFunction,aMarketSession.getMarketName(),aMarketSession.getProducerName());
							...
							aParametersHolder.setSlope(0.5*(myZoneInfoHolder.getConsumption()-myZoneInfoHolder.getExcessDemand()*1.7)/5);
						}
						*/
						demandFunctionParametersList.add(aParametersHolder);					
					}
...
//System.out.println("                 "+myZoneInfoHolder.getName()+" Excess Dem "+myZoneInfoHolder.getExcessDemand()+" Excess Sup "+myZoneInfoHolder.getExcessSupply()+" setting demand for "+aZoneInfoHolder.getName()+" share excess supply "+aZoneInfoHolder.getShareOfExcessSupply());
...
//								tmpIntercept=(int) Math.round((1+Cms_builder.demandFunctionSlopeTuner-Cms_builder.weightOfDistanceInInitializingIntercept)*tmpDoubleValue);
								...
								tmpIntercept=(int) Math.round((1+Cms_builder.demandFunctionSlopeTuner)*Cms_builder.demandFunctionInterceptTuner*tmpDoubleValue);
								if(tmpIntercept<0){
									tmpIntercept=0;
								}
								aParametersHolder=new DemandFunctionParameters(tmpIntercept,aMarketSession.getMarketName(),aMarketSession.getProducerName());
								aParametersHolder.setSlope(Cms_builder.demandFunctionSlopeTuner*tmpDoubleValue/Cms_builder.demandFunctionReferencePrice);

/*
						if(myZoneInfoHolder.getExcessDemand()>0){
							if(aZoneInfoHolder.getExcessSupply()>0){
								tmpDoubleValue=myZoneInfoHolder.getExcessDemand()*Cms_builder.demandFunctionInterceptTuner*aZoneInfoHolder.getShareOfExcessSupply();
								tmpIntercept=(int) Math.round((1+Cms_builder.demandFunctionSlopeTuner-Cms_builder.weightOfDistanceInInitializingIntercept)*tmpDoubleValue);
								if(tmpIntercept<0){
									tmpIntercept=0;
								...
								aParametersHolder.setSlope(Cms_builder.demandFunctionSlopeTuner*tmpDoubleValue/5);
															...
															System.out.println("     Setting intercept: I have excess demand, other country has excess demand");

							}
						}
						else{
							if(aZoneInfoHolder.getExcessSupply()>0){
															System.out.println("     Setting intercept: I have excess supply, other country has excess supply");
...
//								aParametersHolder.setSlope(Cms_builder.demandFunctionSlopeTuner*tmpDoubleValue/5);
								tmpDoubleValue=0.05*aZoneInfoHolder.getExcessSupply();
								aParametersHolder=new DemandFunctionParameters((int)(tmpDoubleValue),aMarketSession.getMarketName(),aMarketSession.getProducerName());							
								aParametersHolder.setSlope(tmpDoubleValue/5);
							}
							else{
															System.out.println("     Setting intercept: I have excess supply "+myZoneInfoHolder.getShareOfExcessSupply()+", other country has excess demand "+aZoneInfoHolder.getShareOfExcessSupply());
							}
						}
						*/
...
//		transportCosts=Cms_builder.transportCostsTuner*((new BigDecimal(distanceFromSellerInKm/100.0)).divide(new BigDecimal(100.0)).setScale(2,RoundingMode.HALF_EVEN)).doubleValue();
		...
		if(parametersHoldeNotFound){
			...
			slopeOfTheDemandFunction=(int)(Cms_builder.demandFunctionSlopeTuner*interceptOfTheDemandFunction/Cms_builder.demandFunctionReferencePrice);
		}
...
//			System.out.println("   "+name+" population "+population+" perCapitaConsumption "+perCapitaConsumption+" periodicConsumptionTarget "+averageConsumption);
			...
			if(Cms_builder.verboseFlag){System.out.println(RepastEssentials.GetTickCount()+"   "+name+" population "+population+" perCapitaConsumption "+perCapitaConsumption+" periodicConsumptionTarget "+averageConsumption);}
//		}
